...
- an optional controller-owned planner / builder / checker lifecycle that
  requires a structured plan, implementation, independent check, and repair
  loop before a coding task can complete;
...
grants company authority. A coding stage cannot complete until every spawned
child has stopped and been explicitly collected. Exact Platform budget stop
codes preserve completed work as interrupted and label unfinished review as
**verification pending** rather than silently accepting the implementation.
Local Qwen remains the on-device draft layer; the
