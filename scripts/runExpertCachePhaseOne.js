...
const outputDir = resolve(
  readOption(args, "--output-dir") ||
    resolve(root, "experiments/expert_cache/output/phase-one")
);
...
const manifest = JSON.parse(
  await readFile(
    resolve(root, "experiments/expert_cache/runtime-manifest.json"),
    "utf8"
  )
);
const hostBefore = {
...
if (ggufs.length === 0 || !trace || !ggufPython || slots.length === 0) {
  console.error(
    "Usage: npm run experiment:expert-phase-one -- " +
...
const layoutArgs = [
  "experiments/expert_cache/build_expert_layout.py",
...
for (const arm of arms) {
  ...
  const replayArgs = [
    "experiments/expert_cache/replay_expert_pages.py",
    "--trace",
    trace,
    "--layout",
    layoutPath,
