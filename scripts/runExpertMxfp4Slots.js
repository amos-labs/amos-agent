...
const source = resolve(
  "experiments/expert_cache/expert_mxfp4_slots.mm"
);
const llamaRoot = resolve(
  readOption(args, "--llama-root") ||
    resolve(homedir(), ".cache/amos/expert-cache/llama.cpp")
);
const buildRoot = resolve(
  readOption(args, "--llama-build") ||
    resolve(llamaRoot, "build-amos-metal")
);
const libraryDir = resolve(buildRoot, "bin");
const buildDir = resolve(
  readOption(args, "--build-dir") ||
    resolve(tmpdir(), "amos-expert-cache-mxfp4")
);
