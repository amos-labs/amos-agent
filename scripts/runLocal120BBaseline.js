...
const outputDir = resolve(readOption(args, "--output-dir") ||
  "experiments/expert_cache/output/live-baseline");
...
const manifest = JSON.parse(await readFile(
  resolve("experiments/expert_cache/runtime-manifest.json"),
  "utf8"
));
...
async function streamingProbe(url, model) {
  ...
  const response = await fetch(`${url}/v1/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: "Reply with exactly: AMOS 120B baseline ready"
      }],
      max_tokens: 32,
      temperature: 0,
      stream: true,
