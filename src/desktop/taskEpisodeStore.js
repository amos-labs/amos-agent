...
export function taskEpisodeEvent(event = {}, { at = new Date().toISOString() } = {}) {
  ...
  if (type === "model_call") {
    return {
      ...
      servedModel: boundedNullable(event.servedModel, 256),
      frontierRoute: boundedNullable(event.frontierRoute, 32),
      correlationId: boundedNullable(event.correlationId, 256),
      ...
      toolCallCount: event.toolCallCount == null ? null : nonNegativeInteger(event.toolCallCount),
