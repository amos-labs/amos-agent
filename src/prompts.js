...
- For a concrete business outcome whose exact AMOS operation is not already available, call amos_resolve_capabilities ONCE in the user's ordinary language. It registers a small typed manifest; use the best matching registered capability next and reuse it until the outcome changes or the manifest expires.
- Do not list engines or load toolkits after a capability manifest answered the request. Use amos_list_engines and amos_load_engine_tools only for manual exploration, compatibility with an older Platform, or when the resolver itself reports no match. Never alternate or repeat discovery calls when no task state changed.
